...
static bool eth_dev_get_mac_address(struct udevice *dev, u8 mac[ARP_HLEN])
{
	...
	if (ret == -ENOSYS)
				ret = 0;
			if (ret){
				printf("\nWarning: %s failed to get MAC address\n",
					   dev->name);
				return false;
				}
			
	memcpy(mac, pdata->enetaddr , ARP_HLEN);
...
static int eth_post_probe(struct udevice *dev)
{
	...
	unsigned char enetaddr_buf[ARP_HLEN + 2];
	char *source = "DT";
...
#if 1
		/* Check if the device has a valid MAC address in efuse */		
	if (eth_dev_get_mac_address(dev, pdata->enetaddr) &&
	    is_valid_ethaddr(pdata->enetaddr)) {
		printf("\nget %s mac address %pM \n",
			       dev->name, pdata->enetaddr);
	    	}
#endif
