...
int pci_bind_bus_devices(struct udevice *bus)
{
	...
	for (bdf = PCI_BDF(dev_seq(bus), 0, 0); bdf <= end;
		...
		dm_pci_read_config(dev, PCI_CLASS_CODE, &class_show, PCI_SIZE_8);
		/*printf("  class code =    0x%.2x (%s)\n", (int)class_show, pci_class_str(class_show));*/

		printf("  %s         %02x:%02x.%02x  - %04lx:%04lx     %s",
				dev->name,PCI_BUS(bdf),PCI_DEV(bdf),PCI_FUNC(bdf),
				vendor, device, bus->name);
		printf("    0x%.2x (%s)\n", (int)class_show, pci_class_str(class_show));
