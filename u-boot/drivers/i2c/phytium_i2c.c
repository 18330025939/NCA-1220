...
#include "phytium_i2c.h"
#include <dm/device_compat.h>
#include <linux/err.h>

/*
 * This assigned unique hex value is constant and is derived from the two ASCII
 * letters 'FT' followed by a 16-bit unsigned number
 */
#define FT_I2C_COMP_TYPE	0x0


static int ft_i2c_enable(struct i2c_regs *i2c_base, bool enable)
{
	...
	do {
		 ...
		 * 25us) as described in the Phytium I2C databook.
		 */
		udelay(25);
	} while (timeout--);
 ...
 * calculations in ft_i2c_calc_timing().
 *
 * @speed: Speed in Hz
...
/**
 * ft_i2c_calc_timing() - Calculate the timings to use for a bus
 *
 ...
 */
static int ft_i2c_calc_timing(struct ft_i2c *priv, enum i2c_speed_mode mode,
			      int ic_clk, int spk_cnt,
			      struct ft_i2c_speed_config *config)
{
	...
	debug("ft_i2c: mode %d, ic_clk %d, speed %d, period %d rise %d fall %d tlow %d thigh %d spk %d\n",
	...
	if (hcnt < 0 || lcnt < 0) {
		debug("ft_i2c: bad counts. hcnt = %d lcnt = %d\n", hcnt, lcnt);
	...
	debug("ft_i2c: hcnt = %d lcnt = %d sda hold = %d\n", hcnt, lcnt,
	      config->sda_hold);
 ...
 */
static int calc_bus_speed(struct ft_i2c *priv, struct i2c_regs *regs, int speed,
			  ulong bus_clk, struct ft_i2c_speed_config *config)
{
	const struct ft_scl_sda_cfg *scl_sda_cfg = NULL;
	...
	if (i2c_spd == IC_SPEED_MODE_HIGH) {
		...
		if ((comp_param1 & FT_IC_COMP_PARAM_1_SPEED_MODE_MASK)
				!= FT_IC_COMP_PARAM_1_SPEED_MODE_HIGH)
			i2c_spd = IC_SPEED_MODE_FAST;
	}
	...
	if (scl_sda_cfg) {
		...
		ret = ft_i2c_calc_timing(priv, i2c_spd, bus_clk, spk_cnt,
					 config);
		if (ret)
...
/**
 * _ft_i2c_set_bus_speed() - Set the i2c speed
 *
 * @priv: Private information for the driver (NULL if not using driver model)
 * @i2c_base: Registers for the I2C controller
 * @speed: Required i2c speed in Hz
 ...
 * @return 0 if OK, -ve on error
 */
static int _ft_i2c_set_bus_speed(struct ft_i2c *priv, struct i2c_regs *i2c_base,
...
int ft_i2c_gen_speed_config(const struct udevice *dev, int speed_hz,
			    struct ft_i2c_speed_config *config)
{
	struct ft_i2c *priv = dev_get_priv(dev);
	ulong rate;
	int ret;
 ...
 */
static int __ft_i2c_read(struct i2c_regs *i2c_base, u8 dev, uint addr,
 ...
 * Write to i2c memory.
 */
static int __ft_i2c_write(struct i2c_regs *i2c_base, u8 dev, uint addr,
			  int alen, u8 *buffer, int len)
{
	int nb = len;
...
/*
 * __ft_i2c_init - Init function
 * @speed:	required i2c speed
 * @slaveaddr:	slave address for the device
 *
 * Initialization function.
 */
static int __ft_i2c_init(struct i2c_regs *i2c_base, int speed, int slaveaddr)
{
	int ret;

	/* Disable i2c */
	ret = ft_i2c_enable(i2c_base, false);
	if (ret)
		return ret;
	...
	_ft_i2c_set_bus_speed(NULL, i2c_base, speed, IC_CLK);
	writel(slaveaddr, &i2c_base->ic_sar);
#endif

	/* Enable i2c */
	ret = ft_i2c_enable(i2c_base, true);
	if (ret)
		return ret;
...
static unsigned int ft_i2c_set_bus_speed(struct i2c_adapter *adap,
					 unsigned int speed)
{
	adap->speed = speed;
	return _ft_i2c_set_bus_speed(NULL, i2c_get_base(adap), speed, IC_CLK);
}

static void ft_i2c_init(struct i2c_adapter *adap, int speed, int slaveaddr)
{
	__ft_i2c_init(i2c_get_base(adap), speed, slaveaddr);
}

static int ft_i2c_read(struct i2c_adapter *adap, u8 dev, uint addr,
		       int alen, u8 *buffer, int len)
{
	return __ft_i2c_read(i2c_get_base(adap), dev, addr, alen, buffer, len);
}

static int ft_i2c_write(struct i2c_adapter *adap, u8 dev, uint addr,
			int alen, u8 *buffer, int len)
{
	return __ft_i2c_write(i2c_get_base(adap), dev, addr, alen, buffer, len);
}

/* ft_i2c_probe - Probe the i2c chip */
static int ft_i2c_probe(struct i2c_adapter *adap, u8 dev)
{
	 ...
	 */
	ret = __ft_i2c_read(i2c_base, dev, 0, 1, (uchar *)&tmp, 1);
	if (ret)
		ft_i2c_init(adap, adap->speed, adap->slaveaddr);

	return ret;
}

U_BOOT_I2C_ADAP_COMPLETE(ft_0, ft_i2c_init, ft_i2c_probe, ft_i2c_read,
			 ft_i2c_write, ft_i2c_set_bus_speed,
			 CONFIG_SYS_I2C_SPEED, CONFIG_SYS_I2C_SLAVE, 0)

#else /* CONFIG_DM_I2C */
/* The DM I2C functions */

static int ft_i2c_xfer(struct udevice *bus, struct i2c_msg *msg,
			       int nmsgs)
{
	struct ft_i2c *i2c = dev_get_priv(bus);
	int ret;
	...
	for (; nmsgs > 0; nmsgs--, msg++) {
		...
		if (msg->flags & I2C_M_RD) {
			ret = __ft_i2c_read(i2c->regs, msg->addr, 0, 0,
					    msg->buf, msg->len);
		} else {
			ret = __ft_i2c_write(i2c->regs, msg->addr, 0, 0,
					     msg->buf, msg->len);
		}
		if (ret) {
...
static int ft_i2c_set_bus_speed(struct udevice *bus, unsigned int speed)
{
	struct ft_i2c *i2c = dev_get_priv(bus);
	ulong rate;
	...
	rate = IC_CLK;
#endif
	return _ft_i2c_set_bus_speed(i2c, i2c->regs, speed, rate);
}

static int ft_i2c_probe_chip(struct udevice *bus, uint chip_addr,
				     uint chip_flags)
{
	struct ft_i2c *i2c = dev_get_priv(bus);
	...
	/* Try to read the first location of the chip */
	ret = __ft_i2c_read(i2c_base, chip_addr, 0, 1, (uchar *)&tmp, 1);
	if (ret)
		__ft_i2c_init(i2c_base, 0, 0);

	return ret;
}

int ft_i2c_of_to_plat(struct udevice *bus)
{
	struct ft_i2c *priv = dev_get_priv(bus);
	int ret;
...
int ft_i2c_probe(struct udevice *bus)
{
	struct ft_i2c *priv = dev_get_priv(bus);
...
#endif
	return __ft_i2c_init(priv->regs, 0, 0);
}

int ft_i2c_remove(struct udevice *dev)
{
	struct ft_i2c *priv = dev_get_priv(dev);
...
const struct dm_i2c_ops ft_i2c_ops = {
	.xfer		= ft_i2c_xfer,
	.probe_chip	= ft_i2c_probe_chip,
	.set_bus_speed	= ft_i2c_set_bus_speed,
};

static const struct udevice_id ft_i2c_ids[] = {
	{ .compatible = "phytium,phytium-i2c" },
	{ }
};

U_BOOT_DRIVER(i2c_ft) = {
	.name	= "phytium-i2c",
	.id	= UCLASS_I2C,
	.of_match = ft_i2c_ids,
	.of_to_plat = ft_i2c_of_to_plat,
	.probe	= ft_i2c_probe,
	.priv_auto	= sizeof(struct ft_i2c),
	.remove = ft_i2c_remove,
