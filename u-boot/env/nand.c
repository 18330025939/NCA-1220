...
static int env_nand_save(void)
{
	...
	env_t   env_new;
	char    *temp_1 = NULL;
	int nand_blk_size = 0x20000;//128k

	ret = env_export(&env_new);
	if (ret)
		return ret;

	ftnand_test_init_entry();//debug

	puts("Erasing Nand Flash...");
	temp_1 = malloc(nand_blk_size);
	memcpy(temp_1, (void *)(CONFIG_ENV_OFFSET - 0x10000), nand_blk_size);

	ret = ftnand_erase_skip_block(CONFIG_ENV_OFFSET - 0x10000, nand_blk_size, 0);
	if(ret != 0)
	{
	...
	memcpy(temp_1 + 0x10000, (void *)&env_new, CONFIG_ENV_SIZE);
	ret = ftnand_skip_write((size_t)(CONFIG_ENV_OFFSET - 0x10000), nand_blk_size, (size_t)temp_1, 0);
	if(ret != 0)
	{
...
#else /*PHYTIUM_NAND*/
	long nand_env_addr;
	nand_env_addr = 0x38000000 + CONFIG_ENV_OFFSET;

	return env_import((char *)nand_env_addr, 1, H_EXTERNAL);
#endif
