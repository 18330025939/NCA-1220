...
perror:
	...
	puts("Erasing Flash...");
	temp_1 = malloc(CONFIG_ENV_SECT_SIZE);
	memcpy(temp_1, (void *)(CONFIG_ENV_ADDR - 0x10000), CONFIG_ENV_SECT_SIZE);
	e2000_lbc_erase_sector(CONFIG_ENV_ADDR, CONFIG_ENV_SECT_SIZE);//128k

	puts("Writing to Flash... ");
	memcpy(temp_1 + 0x10000, (void *)&env_new, CONFIG_ENV_SIZE);
	e2000_lbc_write_word((size_t)temp_1, (size_t)(CONFIG_ENV_ADDR - 0x10000), CONFIG_ENV_SECT_SIZE);
#endif
	return rc;
}
#endif /* CMD_SAVEENV */
...
static int env_flash_load(void)
{
	...
	long lbc_env_addr;
	e2000_lbc_clock_setup();
	lbc_env_addr = 0x10000000 + CONFIG_ENV_ADDR;

	return env_import((char *)lbc_env_addr, 1, H_EXTERNAL);
#endif
}
#endif /* LOADENV */
