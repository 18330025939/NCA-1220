...
static int env_sf_save(void)
{
	...
	qspi_erase_block(CONFIG_ENV_OFFSET);
	ret = env_export(&env_new);
	if (ret)
	{
		printf("export err\n");
	...
	for(index = 0; index < CONFIG_ENV_SIZE; index += 4) {
		value = *(uint64_t *)((uint64_t)(&env_new) + index);
		ret |= qspi_write_word(CONFIG_ENV_OFFSET + index, value);
	}
#endif
...
out:
	...
	{
		buf_env[i] = readb((u64)(CONFIG_ENV_OFFSET+i));
	}
	ret = env_import(buf_env, 1, H_EXTERNAL);
	if (!ret)
	{
		gd->env_valid = ENV_VALID;
	}
